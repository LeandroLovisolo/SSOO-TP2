...
% Booleanos
\newcommand{\PTrue}{\textbf{true} }
\newcommand{\PFalse}{\textbf{false} }
...
\begin{pseudo}
    \Procedure{Modelo::agregarJugador}{nombre}
        ...
        \For{$nuevoid \leftarrow$ 0 \To $max\_jugadores$ - 1}
            ...
            \If{$jugadores[nuevoid] =$ \texttt{NULL}}
...
\Procedure{modelo::finalizar}{}
	\If{$jugando \neq$ \texttt{DISPAROS}} \PReturn{-\texttt{ERROR\_JUEGO\_NO\_COMENZADO}} \EndIf
	\For{$i \leftarrow 0$ To $max\_jugadores$ - 1}
		\If{$jugadores[i] \neq$ \texttt{NULL}}
			\State $evento  \leftarrow$ \Fn{evento}{0, i, 0, 0, \texttt{EVENTO\_START}}
...
\Procedure{termino}{}
    ...
    \For{$i \leftarrow 0$ To $max\_jugadores$ - 1}
    	\State \Fn{rlock}{locks_jugadores[i]}
        \If{!jugadores[i].Fn{termino{}}}
        	\State \Fn{runlock}{locks\_jugadores[i]}
            \State \PReturn{false}
        \EndIf
        \State \Fn{runlock}{locks_jugadores[i]}
    \EndFor
    \State \PReturn{true}
\EndProcedure
\end{pseudo}
...
\Procedure{Modelo::quitarJugador}{s\_id}
	...
	\If{$jugando \neq$ \texttt{SETUP}}
		\State \Fn{runlock}{lock\_jugando}
		\State \PReturn{-\texttt{ERROR_JUEGO_EN_PROGRESO}}
	\EndIf
	\State \Fn{wlock}{locks\_jugadores[s\_id]}
	\If{$jugadores[s\_id]$ = \texttt{NULL}}
		\State \Fn{wunlock}{locks\_jugadores[s\_id]}
		\State \Fn{runlock}{lock\_jugando}
		\State \PReturn{-\texttt{ERROR_JUGADOR_INEXISTENTE}}
	\EndIf

	\State \Fn{delete}{jugadores[s\_id]}
	$jugadores[s_id] \leftarrow$ \texttt{NULL}

	\State \Fn{wunlock}{locks\_jugadores[s\_id]}
	\State \Fm{runlock}{lock\_jugando}
	\State \PReturn{\texttt{ERROR_NO_ERROR}}
\EndProcedure
\end{pseudo}
