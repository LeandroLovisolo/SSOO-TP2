...
\begin{pseudo}
    \Procedure{printPuntaje}{}
        \For{$i \leftarrow$ 0 \To $jugadores\_listos$ - 1}
            \State \Fn{printf}{$"Nombre: \%s - Puntaje: \%d"$, jugadores[i]\to \Fn{dame\_nombre}{}.\Fn{c\_str}{}, jugadores[i]\to \Fn{dame\_puntaje}{}}
        \EndFor
    \EndProcedure
\end{pseudo}
...
\item Se crea la función controlador() para atender las comunicaciones entre un controlador y el servidor.
...
\Procedure{jugador}{nro\_jugador\_ptr}
    \State $nro\_jugador \leftarrow$ * ($int$*) $nro\_jugador\_ptr$
    \State \Fn{delete}{(int*) nro\_jugador\_ptr}
    \State $sale \leftarrow$ false
    \State
...
\Procedure{controlador}{nro\_controlador\_ptr}
    \State $nro\_controlador \leftarrow $* (int*) $nro\_controlador\_ptr$
    \State \Fn{delete}{(int*) nro\_controlador\_ptr}
    \State $sale \leftarrow$ false
    \While{!sale}
