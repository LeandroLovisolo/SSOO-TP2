...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Tests del Readers-Writers Lock                                            %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
La función borrar\_barcos se convierte en privada y no se protege ya que es llamada únicamente desde Modelo::ubicar(), la cual tiene los locks necesarios ya tomados.
\begin{pseudo}
    \Procedure{Modelo::borrar\_barcos}{$t\_id$}
        \If{$jugando \neq$ \texttt{SETUP}}
            \State \PReturn{- \texttt{ERROR\_JUEGO\_EN\_PROGRESO}}
        \EndIf
        \If{$jugadores[t\_id] =$ \Null}
            \State \PReturn{- \texttt{ERROR\_JUGADOR\_INEXISTENTE}}
        \EndIf
        \State quitar $\leftarrow$ jugadores[t\_id].\Fn{quitar\_barcos}{}
        \State \PReturn{quitar}
    \EndProcedure
\end{pseudo}
...
Convertimos la función finalizar en una función privada ya que solo se llama desde Modelo::tocar().
No es necesario realizar locks sobre jugando ya que Modelo::tocar() realiza un lock de escritura sobre esta variable.
...
\begin{pseudo}
    \Procedure{termino}{}
        \State \Fn{rlock}{jugando}
    	\If{$jugando$ = \texttt{SETUP}}
            \State \Fn{runlock}{jugando}
...
\begin{pseudo}
    \Procedure{apuntar}{$s\_id, t\_id, x, y, *eta$}
        ...
        \State Realizamos los locks en orden por número de ID para evitar deadlocks (A ataca B, B ataca A al mismo tiempo) y comprobamos que no se realizen dos locks cuando un usuario se ataca a si mismo.
        \State
        \If{$s\_id < t\_id$}
            \State \Fn{wlock}{locks\_jugadores[s\_id]}
            \State \Fn{wlock}{locks\_jugadores[t\_id]}
...
\begin{pseudo}
    \Procedure{tocar}{$s\_id, t\_id$}
        ...
        \State Como en la función apuntar, realizamos el lock en orden por ID para evitar deadlocks y comprobamos que no se realizen dos locks si el usuario se ataca a si mismo.
        \State
        \If{$s\_id < t\_id$}
            \State \Fn{wlock}{locks\_jugadores[s\_id]}
            \State \Fn{wlock}{locks\_jugadores[t\_id]}
...
\subsection{Modelo::dame\_eta}
\begin{pseudo}
    \Procedure{dame\_eta}{$s\_id$}
        \State Se realiza un lock de jugando para conservar su estado durante la ejecución.
        \State \Fn{rlock}{lock\_jugando}
        \If{$jugando \neq$ \texttt{DISPAROS}}
            \State \Fn{runlock}{lock\_jugando}
            \State \PReturn{- \texttt{ERROR\_JUEGO\_NO\_COMENZADO}}
        ...
        \State Se realiza un lock del jugador, ya que la clase tiros de cada jugador no tiene una protección directa y las únicas funciones que acceden a tiros son apuntar y tocar, previamente habiendo tomado lock del jugador.

        \State \Fn{rlock}{locks\_jugadores[s_id]}

        \If{$jugadores[s\_id] =$ \Null}
            \State \Fn{runlock}{lock\_jugando}
            \State \Fn{runlock}{locks\_jugadores[s\_id]}
            \State \PReturn{- \texttt{ERROR\_JUGADOR\_INEXISTENTE}}
        \EndIf

        \State eta $\leftarrow$ \Fn{getEta}{tiros[s\_id]}
        \State \Fn{runlock}{locks\_jugadores[s\_id]}
        \State \PReturn{eta}
    \EndProcedure
\end{pseudo}
